 ...
 * Handler 는 메인스레드와 작업스레드 간에 통신을 할 수 있는 방법입니다ㅣ !!!!!!!!!!!!!!!!!!!!!!!!!!!!핸들러의 본질
 * <p>
 * 핸들러를 생성한쪽이 받는다
 * Runnable : 간단하게 말해서 Thread의 인터페이스화 된 형태이다
 * 사용 방법:
...
public class Main2Activity extends AppCompatActivity {
    ...
    class BackThread1 extends Thread {
        ...
        public void run() {
            super.run();
            while (true) {
                //첫번째 방법 : 메인에서 생성된 Handler 객체의 sendEmptyMessage 를 통해 Message 전달
                backValue1++;
                handler1.sendEmptyMessage(1);

                //두번째 방법 :  메인에서 생성된 Handler 객체의 postXXX() 를 통해 Runnable 객체 전달
                backValue2 += 2;
                handler2.post(new Runnable() {
                    @Override
                    public void run() { // Runnable 의 run() 에서 메인 UI 접근
                        tvBackValue2.setText("BackValue: "+ backValue2);
                    }
                });
                try {
                    Thread.sleep(1000);
...
// Message 를 수신합니다.
    ...
    Handler handler1 = new Handler() {
        ...
        public void handleMessage(@NonNull Message msg) {
            if (msg.what == 1) {   // Message 가 1 이면
                tvBackValue1.setText("Backvlaue1: " + backValue1); // 메인스레드의 UI 변경
            }
        }
    };
...
class BackThread3 extends Thread{
    ...
    public void run() {
        ...
        while (true){
            ...
            handler.sendMessage(msg); // 메이스레드의 핸들러에  메세지 보내기
...
}// end class BackThread3



//네번째 : 메인스레드의 Handler 를 직접 사용할수 없는 분리된 작업 스레드
