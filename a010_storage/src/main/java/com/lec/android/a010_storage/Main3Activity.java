...
//        db = helper.getReadableDatabase(); // 읽고만 가능한 DB  (SELECT 만 사용하는 경우)
        } catch (SQLException e){
            e.printStackTrace();
            Log.d("myapp", "데이터 베이스르 얻어올 수 없음");
            finish(); // 액티비티 종료
        }
    ...
    void insert(){
        ...
        db.execSQL("INSERT INTO mytable (name) values('임상빈')");
        db.execSQL("INSERT INTO mytable (name) values('윤지우')");
        db.execSQL("INSERT INTO mytable (name) values('장수영')");
        db.execSQL("INSERT INTO mytable (name) values('장윤성')");
